"""AstrBot 定时推送插件 —— 内置群号+用户，无需群内设置。

所有目标群号和用户ID都硬编码在 CONFIG 区域，插件加载后自动注册定时任务。
"""
...
# 目标群列表: { "group_id": 群号(int), "group_name": 群名称, "users": [推送的QQ号列表(str)] }
GROUPS = [
    {
        "group_id": 2156063317,       # <-- 替换为你的实际群号
        "group_name": "我的主力群",
        "users": ["2854203313"],       # <-- 替换为要 @ 的用户QQ号
    },
    # 复制上面的块添加更多群:
    # {
    #     "group_id": 9876543210,
    #     "group_name": "备用群",
    #     "users": ["1000000", "2000000"],
    # },
]
